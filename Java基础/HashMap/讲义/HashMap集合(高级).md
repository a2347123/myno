...
public HashMap(int initialCapacity, float loadFactor) {//initialCapacity=10
  ...
  */
    static final int tableSizeFor(int cap) {//int cap = 10
        //这里如果cap刚刚好是2的整数次幂不减1的话，最后结果每位数上都是1 再加1就相当于乘了2倍
        int n = cap - 1;
        n |= n >>> 1;//至少能将1个1的右边1位数也变成1
        n |= n >>> 2;//接着上面的结果，至少能将2个1右边2位数变成4个1
        n |= n >>> 4;//接着上面的结果，至少能将4个1右边4位数变成4个1
        n |= n >>> 8;//接着上面的结果，至少能将8个1右边8位数变成8个1
        n |= n >>> 16;//接着上面的结果，至少能将16个1右边16位数变成16个1
        
        //如果最终变成了32个1则是负数
        //如果n >= MAXIMUM_CAPACITY也就是说n大于2^30,
        //而在32位的int中2^30已经是最大的2的整数次幂了
...
![image-20220410212047867](HashMap集合(高级).assets/image-20220410212047867.png)
